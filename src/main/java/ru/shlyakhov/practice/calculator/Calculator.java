...
public class Calculator {
    ...
    public boolean isOperator(char c) { // распознавание операторов арифметических действий
        return c == '+' || c == '-' || c == '*' || c == '/';
    }

    public double convertData(String data) { // Конвертирование распознанного числа из String в double
    ...
    public int convertInt(String part) {  // Конвертирование распознанного целого числа или целой части числа из String в int
        int intPart = 0;
    ...
    public double convertDouble(String part) { // Конвертирование распознанной дробной части числа из String в double
        if (part.length() == 1) {
    ...
    public double roundDouble(double number) { // округление числа до 2 знаков после запятой
        number *= 100;
    ...
    public double calculate(double firstNumber, double lastNumber, char operator) { // выполнение арифметических действий
        double sum = 0;
        if (operator == '+') {
    ...
    public String findBrackets(String line) {
        int begin = 0;
    ...
    public void start(String line) throws InputDataException {
        new Validator(line);
